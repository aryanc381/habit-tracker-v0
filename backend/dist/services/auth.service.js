...
export async function signup(input) {
    ...
    if (existingUser) {
        return { status: 409, msg: `User ${input.fullName} is already present in database.` };
    }
    // hashed password
...
export async function login(input) {
    const existingUser = await User.findOne({ email: input.email });
    if (!existingUser) {
        return { status: 409, msg: `User ${input.email} does not exist.` };
    }
    const hashedPassword = existingUser.password;
    const auth = await bcrypt.compare(input.password, hashedPassword);
    if (!auth) {
        return { status: 401, msg: `Invalid credentials for ${existingUser.email}` };
    }
