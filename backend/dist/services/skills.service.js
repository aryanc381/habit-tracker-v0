...
export async function createSkill(input) {
    ...
    if (existingSkill) {
        return { status: 409, msg: `Skill ${input.name} already exists.` };
    }
    ;
...
export async function deleteSkill(input) {
    ...
    if (!skillCheck) {
        return { status: 404, msg: `Skill not found.` };
    }
    ;
...
export async function getSkillByGoalId(input) {
    const goal = getGoalById({ id: toObjectId(input.goalId) });
    const skillIds = (await goal).goalObject?.skillIds;
    const skills = await Promise.all(skillIds?.map((id) => getSkillById({ id: id.toString() })) ?? []);
